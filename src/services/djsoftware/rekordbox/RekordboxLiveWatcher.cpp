...
// Return candidate log file paths for %APPDATA%\Pioneer\rekordbox*.
std::vector<fs::path> candidateLogs()
{
    ...
    if (SHGetFolderPathW(nullptr, CSIDL_APPDATA, nullptr, 0, appData) != S_OK)
        return out;
    fs::path pioneer = fs::path(appData) / L"Pioneer";
    if (!fs::is_directory(pioneer)) return out;
...
std::optional<PlayedTrack> RekordboxLiveWatcher::readFromLogFile()
{
    ...
    if (logs.empty()) {
        spdlog::debug("[RBLive] log-file: no rekordbox log found under %APPDATA%\\Pioneer");
        return std::nullopt;
    }
...
// No-op on non-Windows platforms (BeatMate ships Windows-only for now).
std::optional<PlayedTrack> RekordboxLiveWatcher::readFromWindowTitle() { return std::nullopt; }
std::optional<PlayedTrack> RekordboxLiveWatcher::readFromLogFile()     { return std::nullopt; }
std::optional<PlayedTrack> RekordboxLiveWatcher::readNowPlaying()      { return std::nullopt; }
