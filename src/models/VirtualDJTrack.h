...
struct VirtualDJAutomix {
    std::string type;           // "fade", "cut", "backspin", etc.
    double fadeIn = 0.0;        // seconds
    double fadeOut = 0.0;       // seconds
    double startPosition = 0.0;
    double endPosition = 0.0;

    VirtualDJAutomix() = default;

    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(VirtualDJAutomix,
        type, fadeIn, fadeOut, startPosition, endPosition
    )
};

struct VirtualDJScanData {
    std::string bpmScan;        // VDJ internal BPM scan data
    std::string gainScan;       // VDJ internal gain scan data
    std::string waveformScan;   // VDJ internal waveform data
    bool scanComplete = false;

    VirtualDJScanData() = default;

    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(VirtualDJScanData,
        bpmScan, gainScan, waveformScan, scanComplete
    )
};

struct VirtualDJTrack : public DJSoftwareTrack {
    std::string vdjId;          // VirtualDJ internal ID
    std::map<std::string, std::string> tags;  // VDJ custom tags
    VirtualDJAutomix automix;
    VirtualDJScanData scanData;

    std::string infos;          // VDJ "Infos" field
    std::string firstSeen;      // date first seen
    std::string lastPlay;       // date last played
    int playCount = 0;
    double gain = 0.0;          // auto-gain value (dB)
    std::string flag;           // VDJ flag color
    std::string fileFolder;     // VDJ folder path

    std::vector<std::string> pois;  // serialized VDJ POI data

    VirtualDJTrack() {
        source = TrackSource::VirtualDJ;
    }

    explicit VirtualDJTrack(const std::string& vdjId)
        : vdjId(vdjId) {
        source = TrackSource::VirtualDJ;
    }

    friend void to_json(nlohmann::json& j, const VirtualDJTrack& t) {
        j = nlohmann::json{
    ...
    friend void from_json(const nlohmann::json& j, VirtualDJTrack& t) {
