...
struct RekordboxCue {
    ...
    RekordboxCue() = default;

    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(RekordboxCue,
        number, position, length, name, color, isLoop
    )
};

struct RekordboxMemoryCue {
    double position = 0.0;
    std::string name;
    std::string color;

    RekordboxMemoryCue() = default;

    NLOHMANN_DEFINE_TYPE_INTRUSIVE_WITH_DEFAULT(RekordboxMemoryCue,
        position, name, color
    )
};

struct RekordboxTrack : public DJSoftwareTrack {
    std::string rekordboxId;        // Rekordbox internal ID
    std::vector<std::string> playlistNames;  // playlists this track belongs to
    ...
    std::vector<RekordboxCue> hotCues;
    std::vector<RekordboxMemoryCue> memCues;

    bool beatGridLocked = false;
    std::string color;              // track color in Rekordbox
    int rating = 0;                 // 0-5
    std::string comment;
    std::string comment2;           // Rekordbox has 2 comment fields

    std::string tonality;           // key as displayed in Rekordbox
    std::string mixName;
    std::string remixer;
    ...
    explicit RekordboxTrack(const std::string& rekordboxId)
        : rekordboxId(rekordboxId) {
        source = TrackSource::Rekordbox;
    }

    // JSON manuel à cause de l'héritage
    friend void to_json(nlohmann::json& j, const RekordboxTrack& t) {
        j = nlohmann::json{
    ...
    friend void from_json(const nlohmann::json& j, RekordboxTrack& t) {
